...
function aggregateVolunteerInfo() {
   if (volunteeringData.opportunities.length > 0) {
      ...
      const h1Tag = document.querySelector('#Home h1');
      const h2Tag = document.querySelector('#Home h2');
      
      h1Tag.textContent = opportunity.institution; 
      h2Tag.innerHTML = `<img src="${opportunity.imageUrl}" alt="${opportunity.institution} Image" style="width:100px; height:auto;"> <br>  ${opportunity.name}. <br> For more details: <a href="${opportunity.link}" target="_blank">Click Here</a>. <br> Dates: ${opportunity.dates.join(' until ')}`;
  } else {
